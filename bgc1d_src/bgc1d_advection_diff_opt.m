...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
 ...
 if bgc.FromRestart == 1  
    ...
    no2(1,:) = bgc.rst(7,:);
    n2(1,:)  = bgc.rst(8,:);  
    mytrc(1,:)  = bgc.rst(8,:) * 0.0 + 0.1 ;  % make my tracer equal to 0.1 everywhere
    if bgc.RunIsotopes
       i15no3(1,:)  = bgc.rst(9,:);
       i15no2(1,:)  = bgc.rst(10,:);
       i15nh4(1,:)  = bgc.rst(11,:);	
    ...
    mytrc(1,:) = linspace(bgc.mytrc_top,bgc.mytrc_bot,bgc.npt+1);
    if bgc.RunIsotopes
 ...
 for indt=1:bgc.nt
    ...
    mytrc(2,2:end-1)  = mytrc(1,2:end-1)  .* coeff1 + mytrc(1,3:end)  .* coeff2 + mytrc(1,1:end-2)  .* coeff3;
    if bgc.RunIsotopes
    ...
    mytrc(2,2:end-1) = mytrc(2,2:end-1) + sms.mytrc(2:end-1) * dt;
    if bgc.RunIsotopes
    ...
    if bgc.RestoringOff~=1
       ...
       mytrc(2,2:end-1) = mytrc(2,2:end-1) + restoring.mytrc(2:end-1) * dt;    
       if bgc.RunIsotopes
   ...
   %if mod(indt,bgc.hist) == 0
    if any(bgc.hist_time_ind == indt)
      %iout = indt/bgc.hist; % current output timestep
       ...
       sol(iout,9,:) = mytrc(1,:);
       if bgc.RunIsotopes
          sol(iout,10,:)  = i15no3(1,:);
          sol(iout,11,:) = i15no2(1,:);
          sol(iout,12,:) = i15nh4(1,:);
          sol(iout,13,:) = i15n2oA(1,:);
          sol(iout,14,:) = i15n2oB(1,:);
       end
       ...
       if bgc.flux_diag == 1
          ...
          sadv(iout,9,2:end-1) = alpha .* (mytrc(1,3:end)  - mytrc(1,1:end-2))  + beta .* mytrc(1,2:end-1);
          if bgc.RunIsotopes
             sadv(iout,10,2:end-1)  = alpha .* (i15no3(1,3:end)  - i15no3(1,1:end-2))  + beta .* i15no3(1,2:end-1);
             sadv(iout,11,2:end-1) = alpha .* (i15no2(1,3:end)  - i15no2(1,1:end-2))  + beta .* i15no2(1,2:end-1);
             sadv(iout,12,2:end-1) = alpha .* (i15nh4(1,3:end)  - i15nh4(1,1:end-2))  + beta .* i15nh4(1,2:end-1);
             sadv(iout,13,2:end-1) = alpha .* (i15n2oA(1,3:end) - i15n2oA(1,1:end-2)) + beta .* i15n2oA(1,2:end-1);
             sadv(iout,14,2:end-1) = alpha .* (i15n2oB(1,3:end) - i15n2oB(1,1:end-2)) + beta .* i15n2oB(1,2:end-1);
          end
          ...
          sdiff(iout,9,2:end-1) = gamma .* (mytrc(1,3:end)  - 2 * mytrc(1,2:end-1)  + mytrc(1,1:end-2));
          if bgc.RunIsotopes
             sdiff(iout,10,2:end-1)  = gamma .* (i15no3(1,3:end)  - 2 * i15no3(1,2:end-1)  + i15no3(1,1:end-2));
             sdiff(iout,11,2:end-1) = gamma .* (i15nh4(1,3:end)  - 2 * i15nh4(1,2:end-1)  + i15nh4(1,1:end-2));
             sdiff(iout,12,2:end-1) = gamma .* (i15nh4(1,3:end)  - 2 * i15nh4(1,2:end-1)  + i15nh4(1,1:end-2));
             sdiff(iout,13,2:end-1) = gamma .* (i15n2oA(1,3:end) - 2 * i15n2oA(1,2:end-1) + i15n2oA(1,1:end-2));
             sdiff(iout,14,2:end-1) = gamma .* (i15n2oB(1,3:end) - 2 * i15n2oB(1,2:end-1) + i15n2oB(1,1:end-2));
          end
          ...
          ssms(iout,9,2:end-1) = sms.mytrc(2:end-1);
          if bgc.RunIsotopes
             ssms(iout,10,2:end-1)  = sms.i15no3(2:end-1);
             ssms(iout,11,2:end-1) = sms.i15no2(2:end-1);
             ssms(iout,12,2:end-1) = sms.i15nh4(2:end-1);
             ssms(iout,13,2:end-1) = sms.i15n2oA(2:end-1);
             ssms(iout,14,2:end-1) = sms.i15n2oB(2:end-1);
          end
          ...
          srest(iout,9,2:end-1) = restoring.mytrc(2:end-1);
          if bgc.RunIsotopes
             srest(iout,10,2:end-1)  = restoring.i15no3(2:end-1);
             srest(iout,11,2:end-1) = restoring.i15no2(2:end-1);
             srest(iout,12,2:end-1) = restoring.i15nh4(2:end-1);
             srest(iout,13,2:end-1) = restoring.i15n2oA(2:end-1);
             srest(iout,14,2:end-1) = restoring.i15n2oB(2:end-1);
          end  % bgc.RunIsotopes
       end  % bgc.flux_diag
